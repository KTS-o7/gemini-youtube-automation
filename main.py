...
def produce_lesson_videos(lesson):
    ...
    intro_slide = {
        "title": lesson["title"],
        "content": f"Chapter {lesson['chapter']} | Part {lesson['part']}",
    }
    outro_slide = {
        "title": "Thanks for Watching!",
        "content": "Like, Share & Subscribe for more daily AI content!\n#AIforDevelopers",
    }
    ...
    long_thumb_path = generate_visuals(
        output_dir=OUTPUT_DIR, video_type="long", thumbnail_title=lesson["title"]
    )
...
def main():
    ...
    try:
        ...
        if not pending:
            print(
                "🎉 All lessons produced! Generating new content plan to restart from scratch..."
            )
            ...
            pending = [
                (i, lesson)
                for i, lesson in enumerate(new_plan["lessons"])
                if lesson["status"] == "pending"
            ]
            if not pending:
            ...
            try:
                ...
                if video_id:
                    for original_lesson in plan["lessons"]:
                        if (
                            original_lesson["title"].strip().lower()
                        ...
                        print(
                            f"⚠️ Could not find lesson in plan to mark as complete: {lesson['title']}"
                        )
                else:
